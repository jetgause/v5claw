...
export async function findI18nUntranslatedStrings(
  ...
  const untranslatedStrings = new Map<
    string,
    ...
    try {
      ...
      for (const locale of locales) {
        ...
        if (!existsSync(localeFilePath)) {
          ...
          const allKeys = findI18nUntranslatedKeys(
            englishJson,
            "",
            englishJson,
            {}
          ...
          try {
            ...
            console.error(
              `Error parsing JSON file ${localeFilePath}: ${
          ...
          const untranslated = findI18nUntranslatedKeys(
            englishJson,
            "",
            englishJson,
            localeJson
      ...
      console.error(
        `Error processing file ${englishFilePath}: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
 ...
 */
function findI18nUntranslatedKeys(
  obj: any,
  prefix: string,
  englishObj: any,
  localeObj: any
  ...
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      ...
      if (typeof value === "string") {
        ...
        const nestedUntranslated = findI18nUntranslatedKeys(
          value,
          currentPath,
          englishObj,
          localeObj
...
export async function translateI18nText(
  ...
  console.error(
    `🔤 Translating text to ${targetLanguage}: "${text.substring(0, 30)}${
      text.length > 30 ? "..." : ""
    }"`
  );
...
Translation:`;
    ...
    if (
      ...
      console.error(
        `✅ Translation complete: "${translatedText.substring(0, 30)}${
